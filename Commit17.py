...
class AliasGame:
    ...
    def show_game_turn(self):
        ...
        tk.Label(main_frame, text="Ход игры",
                 font=("Arial", 48, "bold")).pack(pady=30)
        ...
        tk.Label(main_frame, text=turn_description,
                 font=("Arial", 18), justify="left",
                 bg="lightyellow", padx=20, pady=20,
                 relief=tk.RIDGE, borderwidth=2).pack(pady=30, padx=50)
    ...
    def skip_word(self):
        ...
        if self.team_scores[self.current_team] > 0:
            self.team_scores[self.current_team] -= 1
        # Если уже 0, то не уходим в минус
