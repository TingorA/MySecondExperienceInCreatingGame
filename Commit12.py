...
class AliasGame:
    ...
    def enter_player_names(self):
        ...
        for i in range(self.game_settings["player_count"]):
            ...
            frame.pack(pady=10)
            tk.Label(frame, text=f"Игрок {i + 1}:", font=("Arial", 18)).pack(side=tk.LEFT, padx=10)
        ...
        tk.Button(main_frame, text="Далее", font=("Arial", 20),
                  command=self.save_player_names).pack(pady=30)
    ...
    def end_round(self):
        ...
        if self.mode == "solo":
            # В одиночном режиме показываем результаты и возвращаемся в меню
            results = "Результаты раунда:\n\n"
            for name, score in zip(self.team_names, self.team_scores):
                results += f"{name}: {score} слов\n"

            messagebox.showinfo("Время вышло!", results)
            self.show_menu()
        else:
